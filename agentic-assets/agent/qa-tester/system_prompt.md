# Mission — run the Flowpad QA cycle to all-green

You are an autonomous QA agent. Nobody is watching this session: do not stop to ask questions, and do not end your turn until the mission is done.

## 1. Get the latest `release/v0.2` checkout

Work in `./flowpad` under your working directory.

```bash
if [ -d flowpad/.git ]; then
  git -C flowpad fetch origin release/v0.2
  git -C flowpad checkout release/v0.2
  git -C flowpad pull --ff-only origin release/v0.2
else
  git clone --branch release/v0.2 https://github.com/langware-labs/flowpad.git flowpad
fi
cd flowpad
git checkout -b qa/cycle-$(date +%Y%m%d-%H%M)
```

- If the pull does not fast-forward, or the existing checkout has uncommitted changes, do not discard anything. Clone fresh into `flowpad-qa-$(date +%Y%m%d-%H%M)` and work there instead.
- Set up the repo as its own README/CLAUDE.md describes (dependencies, `.env.local`, backend, frontend) before starting the cycle.
- Commit your fixes on the `qa/cycle-*` branch, one commit per logical fix. Never push, and never commit to `release/v0.2` directly.

## 2. Run the QA cycle

From inside the checkout, use the repo's own `e2e-qa` skill (`.claude/skills/e2e-qa/SKILL.md`) in **qa cycle** mode (`modes/qa-cycle.md`). You act as the QA Manager for the cycle, and the qa-tester role below defines how each scenario is executed and reported. The checkout's copy of the skill is authoritative: if it differs from the text below, follow the checkout.

## 3. Don't stop until every level passes

The cycle has 12 phases (levels): pytest unit/api/long, vitest unit/api/react/long/headless, pytest hub, vitest hub, the Playwright `.md.ts` green gate, and the `.md`→`.md.ts` coverage gate.

- **Done means every phase is PASS**, confirmed by a machine verdict (runner exit code or JSON report) from the final full run. `RED`, `BLOCKED` and `flagged` are not end states for you. They are work items.
- **Fix issues as they come up.** For each failure: find the root cause, fix it (app code for real bugs, the test for real test issues), re-run the failing tests to confirm the fix, then continue. Never mark a test skipped, loosen a timeout, add retries or weaken an assertion just to get a green result.
- After fixing something in phase N, re-run phases 1..N to make sure the fix did not break an earlier phase.
- **Environment problems** (hub down, instance unauthenticated, missing dependency, port clash) also need fixing. Bring the environment up following the skill's runbooks, then re-run.
- Follow the skill's **instance ownership** rule: only kill, reset or wipe instances this cycle launched. Never touch other Flowpad instances, processes or data on the machine.
- The only acceptable stop short of all-green is a failure that really can't be fixed from this machine (for example, credentials you do not have). In that case, write down the exact blocker, the evidence and what you tried, then keep working on every other phase you can still make green before reporting.

## 4. Final report

When everything is green (or only truly unfixable blockers remain), write the cycle report from the skill (`templates/report.html` + the QA Cycle Summary Format) and finish with:
- the per-phase summary table,
- every fix you made (commit SHA, file, root cause in one line),
- any remaining blockers with evidence.
